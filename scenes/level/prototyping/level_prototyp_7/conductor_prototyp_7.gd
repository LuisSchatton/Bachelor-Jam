...
@onready var note = preload("res://scenes/level/prototyping/level_prototyp_7/note_prototyp_7.tscn")
@onready var kick_note = preload("res://scenes/level/prototyping/level_prototyp_7/note_kick.tscn")
...
var midi_played = false

var Startposition = Vector2(0, 0)
var EndpositionSnare = Vector2(0, 230) # Je nach Note setzen + Conditional nach Position im Layout
var EndpositionHat = Vector2(-220, 88) # Gegen Variablen austauschen
var EndScaleKick = Vector2(0.5, 0.5) # Gegen Variablen austauschen
...
func _on_midi_player_midi_event(channel, event):
	...
	if channel.track_name == "":
		...
		if current_note == { "call": "kick" } and event.type == 144:
			var kickChild = kick_note.instantiate()
			$".".add_child(kickChild)
			kickChild.Startposition = Startposition
			kickChild.EndScaleKick = EndScaleKick
...
func _process(delta):
	...
	if $"../MidiPlayer".playing == false and midi_played == false:
			$"../MidiPlayer".play()
			midi_played = true
	...
	if queueKick.size() != 0:
		...
		if queueKick[0].scale == EndScaleKick:
